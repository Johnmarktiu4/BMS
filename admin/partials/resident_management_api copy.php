...
switch ($action) {
    ...
    case 'get_residents':
        ...
        $sql = "SELECT *,
                ...
                FROM residents $where ORDER BY id DESC LIMIT $offset, $limit";
        ...
        break;
    /* --------------------------------------------------------------
       2. LIST HEADS (with their members) – used by the Select2 dropdown
       -------------------------------------------------------------- */
    case 'get_heads':
        $page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        ...
        break;
    /* --------------------------------------------------------------
       3. GET ADDRESS OF A HEAD (used when a member selects a head)
       -------------------------------------------------------------- */
    case 'get_head_address':
        ...
        $sql = "SELECT house_number, street, address
                FROM residents
        ...
        break;
    /* --------------------------------------------------------------
       4. GET ONE RESIDENT (for edit / view)
       -------------------------------------------------------------- */
    case 'get_resident':
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
        if ($id <= 0) {
    ...
    case 'update_resident':
        ...
        if ($is_update) {
            ...
            $sql = "INSERT INTO residents (
                ...
                $registered, '$profile_picture', $lat_sql, $lng_sql
            )";
        }
