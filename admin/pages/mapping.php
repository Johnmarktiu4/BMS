    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="mb-0"><i class="fas fa-map me-2"></i>Mapping</h2>
                    <p class="text-muted mb-0">Manage and view all residents address in the barangay</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Barangay Map</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0">
                <div id="locationMap" style="height: 800px;"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    // Center/zoom similar to your OSM URL hash
    map = L.map('locationMap').setView([14.463331, 120.884745], 19);

    // Proper tile layer (tile template with {z}/{x}/{y})
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; OpenStreetMap contributors',
    }).addTo(map);

    // Waypoints derived from your OSM URL’s route param
    const start = L.latLng(14.463799, 120.884792);
    const dest  = L.latLng(latt,  langg);
    ...
    routingControl = L.Routing.control({
      ...
      createMarker: function (i, waypoint, n) {
          // Different color for start vs destination
          const colors = ['#2ecc71', '#e74c3c']; // green for start, red for dest
          return L.circleMarker(waypoint.latLng, {
          radius: 8,
          color: colors[i] || '#34495e',     // stroke color
          weight: 2,
          fillColor: colors[i] || '#34495e', // fill color
          fillOpacity: 1
          });
      }
    }).addTo(map);

    // Ensure map sizes correctly in a modal
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm" role="group">                  
                            <button type="button" class="btn btn-outline-primary" title="View" onclick="openMapModal(${resident.id})"><i class="fas fa-eye"></i></button>
                        </div>
                    </td>
                </tr>`;
                });
