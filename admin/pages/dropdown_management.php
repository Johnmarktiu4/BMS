    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="mb-0"><i class="fas fa-cog m-2"></i>System Dropdown Management</h2>
                    <p class="text-muted mb-0">Manage dropdown options of the system</p>
                </div>
                <div class="d-flex gap-2">
                    <button class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#addResidentModal" onclick="prepareAddResident()">
                        <i class="fas fa-plus me-2"></i>Add Dropdown Options
                    </button>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-4 col-sm-6">
                            <label for="searchInput" class="form-label">Search Dropdown Options</label>
                            <div class="input-group">
                                <span class="input-group-text"><i class="fas fa-search"></i></span>
                                <input type="text" class="form-control" id="searchInput" placeholder="Option item, Category...">
                            </div>
                        </div>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-dark text-white">
                    <h5 class="mb-0"><i class="fas fa-table me-2"></i>Dropdown Options List</h5>
                </div>
                <div class="card-body">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content rounded-3 shadow-lg">
            <div class="modal-header bg-success text-white border-0">
                <h5 class="modal-title" id="addResidentModalLabel"><i class="fas fa-user-plus me-2"></i>Add New Dropdown Options</h5>
                ...
                <form id="addTermForm" enctype="multipart/form-data">
                    ...
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-header bg-white">
                            <h6 class="mb-0"><i class="fas fa-calendar me-2"></i>Dropdown Option Information</h6>
                        </div>
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label for="optionName" class="form-label">Option Name *</label>
                                    <input type="text" class="form-control" id="optionName" name="optionName" required placeholder="e.g, Option 1">
                                </div>
                                <div class="col-md-6">
            ...
            <div class="modal-footer border-0">
                ...
                <button type="button" class="btn btn-primary" id="saveTermBtn" onclick="saveTerm()">Save Dropdown Option</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function prepareAddResident() {
        isEditMode = false;
        $('#addResidentModalLabel').html('<i class="fas fa-calendar me-2"></i>Add New Dropdown Options');
        $('#saveTermBtn').html('Add Dropdown Options').show();
        $('#addTermForm')[0].reset();
        $('#termId').val('');
        $('#lat').val('');
        $('#lng').val('');
    ...
    function editTerm(id) {
        ...
        $('#addTermForm').find('input, select, textarea, button').prop('disabled', false);
        $('.form-check-input').prop('disabled', false);
        $.get('partials/system_settings_api.php', { action: 'get_dropdown', id }, data => {
            if (data.success) {
                const r = data.dropdown;
                $('#addResidentModalLabel').html('<i class="fas fa-user-edit me-2"></i>Edit Dropdown Option');
                $('#saveTermBtn').html('Update Dropdown Option').show();
                $('#termId').val(r.id);
                $('#optionName').val(r.options);
    ...
    function saveTerm() {
        ...
        fd.append('action', isEditMode ? 'update_dropdown' : 'save_dropdown');
        $.ajax({
    ...
    function loadTerms(page = 1) {
        ...
        $.ajax({
            url: 'partials/system_settings_api.php',
            data: { action: 'get_dropdowns', search, status, page, limit },
            dataType: 'json',
            success: function(data) {
                let tbody = '';
                data.dropdowns.forEach(dropdown => {    
                    const statusBadge = dropdown.status == 'Active' ? 'bg-primary' : 'bg-secondary';
                    tbody += `
                <tr>
                    <td><div class="avatar-sm"><strong>${dropdown.id}</strong></div></td>
                    <td>${dropdown.options}</td>
                    <td>${dropdown.category}</td>
                    <td><span class="badge ${statusBadge}">${dropdown.status}</span></td>
                    <td>
                        <div class="btn-group btn-group-sm" role="group">
                            <button type="button" class="btn btn-outline-success" title="Edit" onclick="editTerm(${dropdown.id})"><i class="fas fa-edit"></i></button>
                        </div>
                    </td>
                </tr>`;
                });
