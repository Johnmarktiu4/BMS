...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h2 class="mb-0"><i class="fas fa-undo me-2"></i>Returned Items</h2>
                <p class="text-muted mb-0">Track items returned to the inventory</p>
            </div>
            <button class="btn btn-primary btn-lg" onclick="openTransactionModal(0,'Return')">
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-dark text-white">
                    <h5 class="mb-0"><i class="fas fa-history me-2"></i>Returned Items</h5>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover" id="returnedTable">
                            ...
                            <tbody id="returnedTableBody"></tbody>
                        </table>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content rounded-3 shadow-lg">
                <div class="modal-header bg-success text-white border-0">
                    <h5 class="modal-title"><i class="fas fa-exchange-alt me-2"></i>Return Item</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4 bg-light">
                    <form id="transactionForm">
                        <input type="hidden" name="inventory_id" id="transactionItemId">
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select" name="inventory_id" id="itemSelect" required>
...
function loadReturnedItems() {
    $.post('partials/inventory_management_api.php', { action: 'fetch_transactions' }, r => {
        if (r.status === 'success') {
            const list = r.data.filter(x => x.action_type === 'Return');
            updateReturnedTable(list);
        } else showAlert('danger', r.message || 'Failed');
    }, 'json');
}

function updateReturnedTable(data) {
    const $tbody = $('#returnedTableBody').empty();
    if (!data.length) return $tbody.append('<tr><td colspan="7" class="text-center">No returned items.</td></tr>');
...
function openTransactionModal(id, type) {
    ...
    $('[name=return_date]').val(''); $('[name=remarks]').val('');
    loadItemsForSelection();
    $('#transactionModal').modal('show');
}

// Auto-fill borrower when item is selected (optional)
$('#itemSelect').on('change', function() {
    const iid = $(this).val();
    if (!iid) return;
    $.post('partials/inventory_management_api.php', { action: 'fetch_transactions' }, r => {
        if (r.status === 'success') {
            const borrow = r.data.find(t => t.inventory_id == iid && t.action_type === 'Borrow');
            if (borrow && borrow.borrower_name) {
                $('#borrowerSearchInput').val(borrow.borrower_name);
                $('#selectedBorrowerName').text(borrow.borrower_name);
                // Optionally auto-set borrower_id if exists
            }
        }
    }, 'json');
});
    ...
    $.ajax({
        ...
        success: r => {
            ...
            if (r.status === 'success') {
                ...
                showAlert('success', 'Return recorded!');
                loadReturnedItems();
...
function loadItemsForSelection() {
    $.post('partials/inventory_management_api.php', { action: 'fetch_inventory', page: 1, limit: 9999 }, r => {
        ...
        if (r.status === 'success') r.data.items.forEach(i => {
            s.append(`<option value="${i.id}">${i.item_name} (Stock: ${i.current_stock})</option>`);
        });
    }, 'json');
}
